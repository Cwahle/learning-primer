...
| GIAC GSE certification (long-range goal) | New | The destination the other tracks feed into (set 2026-08-03). GSE requires GSEC + GCIH + GCIA (Gold in one) before sitting the two-day hands-on lab, so this is a multi-year path, not a next exam. Nearest-term contributors already in motion: Linux/CLI and Security/CTF (→ GSEC), Networking & infrastructure (→ GCIA — currently the weakest link and a confirmed diagnostic gap). No GIAC-specific material started yet. When teaching, favor hands-on/practical framing — GIAC exams are open-book and the GSE lab is entirely practical. | 2026-08-03 |
